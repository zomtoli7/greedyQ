...
**Current release identifier:** `0.2_2026-09-09_fcb3c86`
...
- Apply page navigation policy exactly: Previous and Next may be shown, hidden, or disabled; Next may have a declared activation delay. Terminal pages have no navigation. Return a changed survey pane to its top, keep stacked mobile preview panes at no more than 390px, and avoid empty mobile scroll space.
- Select from the canonical 16 surveydown controls rather than inventing widgets: `text`, `textarea`, `numeric`, `mc`, `mc_multiple`, `mc_buttons`, `mc_multiple_buttons`, `mc_image`, `mc_multiple_image`, `select`, `slider`, `slider_numeric`, `date`, `daterange`, `matrix`, and `matrix_multiple`. The fixed greedyQ extensions `audio` and `video` may be used for media stimuli. Use the fixed parser and renderer; do not recreate control code per study.
- Advanced greedyQ extensions are `rank_order`, `side_by_side`, `nps`, `timing`, `constant_sum`, `pick_group_rank`, and `drill_down`. Prefer these fixed controls over a custom control.
- If the researcher requests a custom control, explain before work begins that it may require more AI usage and time. Ask one focused question at a time until appearance, interaction, validation, mobile behavior, accessibility, and stored data are explicitly agreed. Record that agreement, choose the closest supported `base_type`, and only then instantiate `type = "custom"`. Never silently invent custom behavior.
- Re-evaluate conditional visibility immediately when its source answer changes. On mobile, render matrix rows as independent response cards and split scales into groups of two or three columns without horizontal page scrolling.
...
Before an external connection, run the equivalent of `preregister`, `export-surveydown`, and `preflight`. Preregistration output remains an unapproved local draft. Native export must report every material mismatch and must not claim equivalence when greedyQ-only behavior remains. Supabase setup applies the three canonical migrations in numeric order: `001_initial.sql` for tables, RLS, withdrawal, and analysis boundaries; fixed `002_browser_rpc.sql` for capability-token sessions, consent-gated writes, and locked assignment; and fixed `003_results_dashboard.sql` for the protected researcher results boundary. Build must record all migration hashes in `supabase/migrations/manifest.json`, and preflight must stop on a missing file, duplicate number, or checksum mismatch. Never edit an applied migration or invent alternate security RPCs; add the next numbered migration instead.
