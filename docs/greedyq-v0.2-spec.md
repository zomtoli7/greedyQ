...
greedyq:
  spec_version: "0.2"
  version: "0.2_2026-09-09_236d2a3"
...
Advanced greedyQ-only controls are `rank_order` (item-to-rank map), `side_by_side` (column-to-row response maps), `nps` (bounded 0–10 response by default), `timing` (hidden seconds-on-page record), `constant_sum` (item allocation map with a declared target), `pick_group_rank` (item-to-group-and-rank map), and `drill_down` (ordered hierarchical path). These are independent greedyQ implementations inspired by common survey interactions; they do not claim binary or data-format compatibility with Qualtrics.

`custom` MUST declare a supported `base_type`, a plain-language `customization` agreement, and may declare a namespaced `custom_class`. The agent MUST disclose additional time/usage, complete a researcher dialogue covering visual behavior, interaction, validation, responsive behavior, accessibility, and stored values, and receive confirmation before generating it. A custom control MUST extend a fixed base control rather than replace the deterministic parser or silently inject arbitrary script.
