...
완성된 연결 경험에서는 Vercel 로그인과 Vercel-native Supabase 리소스 승인 한 번 뒤 연구자에게 공개 설문과 보호된 결과 대시보드 링크 두 개를 제공합니다. Supabase는 infrastructure이며 일상적인 연구자 interface가 아닙니다. 대시보드는 테스트와 실제 응답, Direct와 Prolific 모집을 구분합니다.
