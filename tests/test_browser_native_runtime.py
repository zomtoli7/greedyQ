...
console.log(JSON.stringify({saved,conflict,refreshed}));'''
    ...
    def test_browser_prolific_contract_and_stable_session_support(self):
        ...
        self.assertIn("resolveProlificLaunch(location.search", "".join(respondent.split()))
        ...
        direct = self.node('const gq=require("./web/greedyq-core.js");console.log(JSON.stringify(gq.resolveProlificLaunch("","test")));')
    ...
    def test_supabase_bridge_creates_session_and_registers_external_identifiers(self):
        ...
        self.assertIn('state.lifecycle === "active"', core)
        self.assertIn('state.consent_accepted', core)
    ...
    def test_accessibility_and_mobile_contract_is_present(self):
        ...
        self.assertIn("data-save-error", html)
        self.assertIn('root.querySelector("[data-save-error]")', html)
