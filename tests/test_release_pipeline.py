...
class ReleasePipelineTests(unittest.TestCase):
    ...
    def test_exporter_generates_runnable_shape_and_truthful_mismatches(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertRegex(exported_qmd, r"(?m)^format: html$")
            self.assertIn("library(surveydown)", exported_qmd)
            self.assertRegex(exported_qmd, r"survey-settings:\n\s+mode: preview")
            numeric = re.search(r'sd_question\(\s*\n?\s*id\s*=\s*"age".*?\)', exported_qmd, re.S).group(0)
            self.assertNotIn("min =", numeric)
            self.assertNotIn("max =", numeric)
            self.assertNotIn("previous_mode", exported_qmd)
            self.assertNotIn("next_mode", exported_qmd)
            self.assertNotIn("next_delay_seconds", exported_qmd)
            self.assertNotIn(")```", exported_qmd)
            ...
            server_body = app.split("server <- function", 1)[1].split("shiny::shinyApp", 1)[0]
            self.assertGreater(server_body.rfind("sd_server(db = db)"), server_body.rfind("sd_stop_if("))
    ...
    def test_exporter_translates_greedyq_controls_to_surveydown_custom_controls(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for qid in ("rank_example", "sbs_example", "nps_example", "timing_example", "sum_example", "group_rank_example", "drill_example", "audio_example", "video_example", "custom_example"):
                self.assertIn(f'sd_output("{qid}", type = "question")', qmd)
                self.assertIn(f'id = "{qid}"', app)
                self.assertIn(f'output = uiOutput("gq_{qid}_output")', app)
                self.assertIn(f'value = gq_{qid}_value', app)
            ...
            self.assertGreaterEqual(qmd.count('type = "question"'), 10)
            self.assertGreaterEqual(app.count("sd_question_custom("), 10)
